	...
	methods: {
		...
		async uploadFile(index = 0) {
			...
			if(this.beforeUpload && typeof(this.beforeUpload) === 'function') {
				...
				// 非页面的this，所以这里需要往上历遍，一直寻找到最顶端的$parent，这里用了uni.$u.$parent.call(this)
				// 明白意思即可，无需纠结uni.$u.$parent.call(this)的细节
				let beforeResponse = this.beforeUpload.bind(uni.$u.$parent.call(this))(index, this.lists);
				// 判断是否返回了promise
			...
			const task = uni.uploadFile({
				...
				success: res => {
					...
					let data = this.toJson && uni.$u.test.jsonString(res.data) ? JSON.parse(res.data) : res.data;
