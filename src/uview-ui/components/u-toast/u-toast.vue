		...
		computed: {
			iconName() {
				...
				if (['error', 'warning', 'success', 'info'].indexOf(this.config.type) >= 0 && this.config.icon) {
					let icon = uni.$u.type2icon(this.config.type);
					return icon;
				}
			},
			uZIndex() {
				// 显示toast时候，如果用户有传递z-index值，有限使用
				return this.isShow ? (this.zIndex ? this.zIndex : uni.$u.zIndex.toast) : '999999';
			}
		},
		methods: {
			// 显示toast组件，由父组件通过this.$refs.xxx.show(options)形式调用
			show(options) {
				this.config = uni.$u.deepMerge(this.config, options);
				if (this.timer) {
					// 清除定时器
				...
				if (this.config.url) {
					...
					if (Object.keys(this.config.params).length) {
						...
						if (/.*\/.*\?.*=.*/.test(this.config.url)) {
							// object对象转为get类型的参数
							query = uni.$u.queryParams(this.config.params, false);
							this.config.url = this.config.url + "&" + query;
						} else {
							query = uni.$u.queryParams(this.config.params);
