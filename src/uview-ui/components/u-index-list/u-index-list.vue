...
<script>
	...
	export default {
		...
		methods: {
			...
			setListRect() {
				return uni.$uGetRect('.u-index-bar').then((rect) => {
					Object.assign(this, {
						height: rect.height,
			...
			setSiderbarRect() {
				return uni.$uGetRect('.u-index-bar__sidebar').then(rect => {
					this.sidebar = {
						height: rect.height,
						top: rect.top
					};
						...
						if (index === active) {
							...
							if (isActiveAnchorSticky) {
								...
								anchorStyle = {
									...
									zIndex: `${zIndex ? zIndex : uni.$u.zIndex.indexListSticky}`,
									color: `${activeColor}`
								};
							}
							item.active = active;
